...
// Get monthly reward data for chart
$stmt = $conn->prepare("
...
$stmt->execute();
$monthlyData = array_reverse($stmt->get_result()->fetch_all(MYSQLI_ASSOC));

// Get problem categories funded
$stmt = $conn->prepare("
...
$stmt = $conn->prepare("
    SELECT sdg_focus, COUNT(*) as count
    FROM challenges WHERE sponsor_id = ? AND sdg_focus IS NOT NULL
    GROUP BY sdg_focus
");
if (!$stmt) {
    die("Database error: " . $conn->error);
...
$stmt->bind_param("i", $user['id']);
$stmt->execute();
$sdgData = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
            ...
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center space-x-3">
                    <a href="index.php" class="flex items-center space-x-2">
                        ...
                        <span class="font-bold text-xl text-gray-800">Civic <span class="text-blue-600">Ledger</span></span>
                    </a>
                </div>
        ...
        <div class="grid lg:grid-cols-2 gap-8 mb-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto">
                    ...
                    $stmt->bind_param("i", $user['id']);
                    $stmt->execute();
                    $sdgDist = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
                    ...
                    <?php endforeach; ?>
                    <?php if (empty($sdgDist)): ?>
                    <p class="text-gray-400 text-center py-8">No SDG data yet</p>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3 max-h-80 overflow-y-auto">
                    ...
                    <p class="text-gray-400 text-center py-8">No activity recorded yet</p>
                    <?php else: ?>
                    ...
                    <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-800"><?= e($log['description']) ?></p>
                            <p class="text-xs text-gray-400"><?= timeAgo($log['created_at']) ?></p>
                        </div>
                    </div>
                    <?php endforeach; ?>
                    <?php endif; ?>
