<?php
/**
 * CivicLedger - Mentor Solution Approval
 * Mentors approve solutions from their assigned teams
 */
...
// Get pending solutions from mentor's assigned teams
$stmt = $conn->prepare("
    SELECT s.*, u.name as author_name, u.id as author_id, c.title as challenge_title, t.name as team_name
    FROM solutions s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($solutionId && in_array($action, ['approve', 'reject'])) {
        ...
        $stmt = $conn->prepare("UPDATE solutions SET status = ?, mentor_feedback = ? WHERE id = ?");
        if (!$stmt) {
            die("Database error: " . $conn->error);
        }
        $stmt->bind_param("ssi", $status, $feedback, $solutionId);
        if ($stmt->execute()) {
            setFlash('success', 'Solution ' . $status . ' successfully!');
            header("Location: mentor_solution_approval.php");
            exit;
        }
    }
}
...
<head>
    ...
    <title>Solution Approval - CivicLedger Mentor</title>
        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center space-x-4">
                    ...
                    <a href="mentor_problem_approval.php" class="px-4 py-2 text-gray-300 hover:bg-gray-800 rounded-lg"><i class="fas fa-file-alt mr-1"></i>Problems</a>
                    <a href="mentor_solution_approval.php" class="px-4 py-2 bg-green-600 text-white rounded-lg"><i class="fas fa-check mr-1"></i>Solutions</a>
    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h2 class="text-xl font-bold text-gray-900">Pending Solutions from Your Teams</h2>
            ...
            <div class="divide-y">
                ...
                <div class="p-6 hover:bg-gray-50 card-hover transition-all">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wide">Team</p>
                            <p class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($solution['team_name']); ?></p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900"><?php echo date('M d, H:i', strtotime($solution['created_at'])); ?></p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wide">Budget Estimate</p>
                            <p class="text-sm font-bold text-green-600">Rs. <?php echo number_format($solution['budget_estimate'], 0); ?></p>
                        </div>
                    </div>

                    <div class="mb-4 p-4 bg-gray-50 rounded border-l-4 border-blue-500">
                        <p class="text-xs font-semibold text-gray-600 mb-2"><i class="fas fa-quote-left mr-1"></i>Solution:</p>
                        <p class="text-sm text-gray-700"><?php echo substr(htmlspecialchars($solution['description']), 0, 300); ?><?php echo strlen($solution['description']) > 300 ? '...' : ''; ?></p>
                    </div>

                    <?php if (!empty($solution['implementation_plan'])): ?>
                    <div class="mb-4 p-4 bg-amber-50 rounded border-l-4 border-amber-500">
                        <p class="text-xs font-semibold text-amber-700 mb-2"><i class="fas fa-map mr-1"></i>Implementation Plan:</p>
                        <p class="text-sm text-amber-700"><?php echo substr(htmlspecialchars($solution['implementation_plan']), 0, 300); ?></p>
                    </div>
                    <?php endif; ?>

                    <div class="flex space-x-3">
                        <button onclick="showApprovalModal(<?php echo $solution['id']; ?>, 'approve', '<?php echo htmlspecialchars($solution['author_name']); ?>')" class="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium text-sm">
                            <i class="fas fa-check mr-2"></i>Approve
                        </button>
                        <button onclick="showApprovalModal(<?php echo $solution['id']; ?>, 'reject', '<?php echo htmlspecialchars($solution['author_name']); ?>')" class="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium text-sm">
            ...
            <div class="px-6 py-12 text-center">
                ...
                <p class="text-sm text-gray-500 mt-2">Check back later for more submissions from your teams.</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-md w-full mx-4">
            ...
            <form method="POST" class="px-6 py-4 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="feedback" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="Provide constructive feedback for the student..."></textarea>
                </div>
